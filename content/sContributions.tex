...
over 3 million tags where stored in the system's database.
...
Therefore, a more ideal solution to querying the data was looked for.

\subsubsection{Setting up an Amazon EMR Spark Cluster for querying the Tags table}
This led to Amazon EMR Spark Cluster \todo{Describe Amazon EMR} as a platform for loading all of the table in memory for querying.
...
After Amazon provisioned and started the nodes in the cluster, logging into the cluster was a matter of using
\textit{ssh} and the set key pair at cluster creation. 
